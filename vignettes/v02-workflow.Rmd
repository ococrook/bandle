...
```{r plotres, fig.height=14, fig.width=5}
par(mfrow = c(5, 2))
...
<!-- We also combine the replicates of each condition -->
<!-- for purpose of plotting. -->

<!-- ```{r cmbdata, warning=FALSE} -->
<!-- res_0h[[2]] <- updateFvarLabels(res_0h[[2]], label = "data2") -->
<!-- res_12h[[2]] <- updateFvarLabels(res_12h[[2]], label = "data2") -->

<!-- res_0h_cmb <- combine(res_0h[[1]], res_0h[[2]]) -->
<!-- res_12h_cmb <- combine(res_12h[[1]], res_12h[[2]]) -->

<!-- res_all <- list(res_0h_cmb, res_12h_cmb) -->
<!-- ``` -->
...
Let's plot the profiles of a few of the proteins with the highest
differential localisation scores,
...
hours-LPS is plasma membrane. This fits with the information we see on Uniprot
which tells us it is Small integral membrane protein 1 (SMIM1).

In the below code chunk we plot the protein profiles of proteins identified as
lysosomal from BANDLE in the control and then overlay SMIM1. We do the same at
12hrs post LPS with all plasma membrane proteins.
...
plotDist(res_0h[[1]][lyso], pcol = cols["Lysosome"], alpha = 0.1)
...
plotDist(res_12h[[1]][pm], pcol = cols["Plasma Membrane"], alpha = 0.1)
