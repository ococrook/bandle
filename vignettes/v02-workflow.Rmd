...
measurements where we have proteins/peptides along the rows, and
samples/fractions along the columns. To use `bandle` the data must be stored as
a `MSnSet`, as implemented in the Bioconductor `r Biocpkg("MSnbase")` package.
Please see the relevant vignettes in `r Biocpkg("MSnbase")` for constructing
these data containers.
...
localisation probabilities. The 7 columns are
...
majority of unlabelled proteins to the nucleus (please note, this results is
irrespective of the posterior probability assigned to the protein and thus
includes low confidence assignments).
