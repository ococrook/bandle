...
## Example data: spatialtemporal proteomic profiling of a THP-1 cell line

The data used in this vignette has been published in @thplopit and is stored as
`MSnSet` instances in `r Biocpkg("pRolocdata")` package. 
...
across 20 channels. The data is accessed through different slots of the `MSnSet`
(see `str(thpLOPIT_unstimulated_rep1_mulvey2021)` for all available slots). The
3 main slots which are used are those the contain the quantitation data,
features i.e. PSM/peptide/protein information, and sample information, and these
can be accessed using the functions `exprs`, `fData`, and `pData`, respectively.
...
We now have our list of `MSnSet`s
...
Here for convience of the vignette we only run 2 of the 3 replicates and
furthermore, iterations are typically $1000$s. Again, a few are run here for the 
convenience of the vignette run-time.
...
We create a new object from populating the `bandleres` objects using the `summaries` function.
...
The differential localisation probability is also stored here, which tells us
which proteins are most likely to differentially localise. The rank plot is a
good visualisation. Indicating that most proteins are not differentially
localised and there are a few confident differentially localised protiens of
interest.
...
We see that all of these top 100 proteins have a posterior distribution of 1. We
can plot the top 1000 to see how these distributions compare.
